...
class Railway
  ...
  def menu
    ...
    puts "Введите 12 для просмотра списка вагонов у поезда"
    puts "Введите 13 для заполнения вагонов пассажирами или грузом"
  ...
  def new_coach
    ...
    if coach_choice == 1
      ...
      @coaches << coach
      puts "В депо добавлен новый ГРУЗОВОЙ вагон на #{coach.free_space} литров"
    elsif coach_choice == 2
      puts "Введите число мест в вагоне"
      seat = gets.to_i
      coach = PassengerCoach.new(seat)
      @coaches << coach
      puts "В депо добавлен новый ПАССАЖИРСКИЙ вагон на #{coach.free_space} мест"
    else
      puts "Ошибка ввода"
      return
    end
  ...
  def coaches_list
    @coaches.each_with_index { |coach, index| puts "#{index + 1}) #{coach.type} #{coach.free_space}" }
  end
  ...
  def train_coaches_list
    ...
    @num = gets.to_i
    @trains[@num - 1].train_coaches_list do |coach, index|
      puts "Вагон №#{index + 1} #{coach.type}; Свободный объем: #{coach.free_space} литров; Занятый объем: #{coach.busy_space} литров;" if coach.class == CargoCoach
      puts "Вагон №#{index + 1} #{coach.type}; Свободно: #{coach.free_space} мест; Занято: #{coach.busy_space} мест;" if coach.class == PassengerCoach
    end
  end
